...
`nerdMode` is an opt-in display flag (off by default, persisted in `wattlyzer_settings`, never part of `toSchedulingSettings`): when on, home shows the recommended slot's averages under the reason chip. `nerdStats` in `apps/pwa/src/components/sky/home.tsx` decides what is showable and gates on `bestSlotMode`, not field presence — `price-only` carries an `avgSolarProduction` it never waited for, and `solar-only` has no price. `avgSolarProduction` is average Wh per hour after the fixed 0.7 factor (same scale as `minKwh`), displayed as kWh with one decimal; `avgPrice` is the raw aWATTar `marketprice` mean in Eur/MWh and may only be converted to ct/kWh through `marketPriceToCentsPerKwh` in `@wattlyzer/core`.
