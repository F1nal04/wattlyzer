...
**Cross-domain links.** Website install and open-app links use `https://pwa.wattlyzer.de` with `?lang=en` or `?lang=de`. The PWA consumes this explicit choice once after the root mounts and persists it through `setLocale`; SSR and the first client render stay English, and subsequent Settings choices can override it. Settings links back to `https://wattlyzer.de/` or `/de/` using the active locale.
