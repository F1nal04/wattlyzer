...
- **Glass boxes (`backdrop-filter`) inside an `overflow`-scroll container need `transform: translateZ(0)`.** iOS Safari defers painting a `backdrop-filter` element nested in an overflow-scroll container until a scroll forces a recomposite — over a dark sky the box and its text stay blank until you scroll. Promoting each such box to its own GPU layer makes it paint on first render. Applies to `SetGroup` (`settings.tsx`), `TextCard` (`text-page.tsx`), and `ShadingItem` (`solar-modal.tsx`); add the same hint to any new frosted box rendered inside a scroll view. Onboarding is the exception: its sky palette changes while the cards remain mounted, and macOS Safari caches the old sky inside backdrop-filter layers. Keep onboarding glass filter-free and use the translucent theme fill instead.
